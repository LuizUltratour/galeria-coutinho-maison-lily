(function (w, d) {
  ...
  var BASE = 'https://skylineip.s3.sa-east-1.amazonaws.com/Tour+Virtual/COUTINHO/galeria-maison-lily/';
  ...
  function _open(mode) {
    _mount(GALLERY_URL + '?mode=' + encodeURIComponent(mode || 'imagens') + '&v=' + Date.now(), '#646461');
  }
  ...
  w.AbrirGaleriaVideos = function (show) {
    if (show === 1) _mount(VIDEO_GALLERY_URL + '?v=' + Date.now(), '#1c1c1a', 'fullscreen; autoplay');
    else _close();
  };
